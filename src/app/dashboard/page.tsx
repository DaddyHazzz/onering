      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-10 shadow-2xl">
          ...
          <button
            ...
            className="mt-8 px-12 py-6 bg-purple-600 hover:bg-purple-500 disabled:opacity-50 disabled:cursor-not-allowed rounded-2xl text-2xl font-bold transition transform hover:scale-105 active:scale-95"
          >
    ...
    <div className="mt-6 flex flex-wrap gap-4">
      <button
        onClick={async () => {
          ...
          if (data.success) {
            alert(`Posted live on X!\n${data.url}`);
          } else {
            alert(`X error: ${data.error}\n\nCopy-paste for now — approval usually comes in <24h`);
          }
        }}
        disabled={loading}
        className="px-10 py-4 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 disabled:opacity-50 rounded-xl text-2xl font-black transition transform hover:scale-105"
      >
        {loading ? "Posting to X…" : "Post to X Now"}
      </button>
